    ...
    def __init__(self, d_model, dim, depth, n_head, dropout = 0.5):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                # ConvAttention(dim, d_model, n_head, dropout),
                # MBConv(dim, dropout=dropout)

                # PreNorm(d_model, MultiHeadAttention(d_model, 2, dropout)),
...
class LiteViT(nn.Module):
    def __init__(self, dim, emb_size, n_classes, n_head=1, n_layers=1, dropout=0.5):
        ...
        self.conv_t = nn.Sequential(
            ...
            nn.Conv2d(dim, emb_size, (10, 1), stride=(10, 1), bias=False),
            # Rearrange('b c h w -> b h w c'),
            # nn.LayerNorm(emb_size)
        ) 
        ...
        # )
        # self.transformer_t = Transformer(22, dim, 1, 2, dropout)
        self.transformer_s = Transformer(100, dim, 1, n_head, dropout)
        # nn.Sequential(
        ...
        #     # ConvAttention(dim, 1000, n_head, dropout)
        # )
        ...
        self.cls = nn.Sequential(
            ...
            # SeparableConv2d(dim, n_classes, (10, 22), bias=False, depth_multiplier=4),
            # nn.BatchNorm2d(n_classes),
            # nn.ELU(),
            # nn.Dropout(dropout),

            nn.Conv2d(dim, n_classes, (10, 22), bias=False),
            # Conv2dNormWeight(dim, n_classes, (10, 22), max_norm=0.5),
            nn.BatchNorm2d(n_classes),
            # nn.Dropout(dropout),
        ...
        self.pos_embedding_s = PositionalEmbedding(100, 22)
    ...
    def forward(self, x):
        ...
        # t = rearrange(t, 'b (c h) w -> b c h w', c=1)
        # x = t #+ t
        # merge
        x = s
        # classification
...
model = LiteViT(1, 1, 4, n_head=10, n_layers=1)
x = model(x)
print(x.size())
