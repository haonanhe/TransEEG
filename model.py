...
# from conv_model import CSPFeedForward, ConvPositionwiseFeedForward
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, max_len, d_model, n_head, dropout=0.1, activation="relu",
        ...
        #     #                                    dropout=dropout)
        #     self.feed_forward = ConvPositionwiseFeedForward(50)
...
class NaiveTransformer(nn.Module):
    def __init__(self, max_len, d_model, n_classes, dropout = 0.1,
        ...
        self.pooling = nn.AvgPool2d((1, 40), (1, 40))
        ...
        self.classifier = nn.Sequential(
            ...
            nn.LayerNorm(max_len//40),
            nn.Linear(max_len//40, n_classes),
            nn.LogSoftmax(dim=1)
...
# model = NaiveTransformer(1000, 22, 4, n_head=2)
# x = model(x)
# print(x.size())
...
class PatchEmbedding(nn.Module):
    def __init__(self, n_c, patchsize, d_model):
        super(PatchEmbedding, self).__init__()
        patch_height, patch_width = patchsize[0], patchsize[1]
        patch_dim = n_c * patch_height * patch_width
...
class CSPViT(nn.Module):
    def __init__(self, n_timepoints, n_channels, n_classes, 
                 dropout = 0.1, n_head = 1, num_layers = 2, 
                 filter_size_time=25, filter_size_spatial=22,
                 n_filter_time=4, n_filter_spatial=2,
                 patchsize=(25, 1)):
        super().__init__()
        ...
        # CSP projection
        self.csp = CSPProjection(filter_size_time, filter_size_spatial, n_filter_time, n_filter_spatial)
        self.n_timepoints = n_timepoints
        self.n_channels = 1

        # patch embedding
        n_c = n_filter_time*n_filter_spatial
        self.patch_embedding = PatchEmbedding(n_c, patchsize, patch_height)
